...
class Location:
    ...
    def moveUntilObstacle(self):
        ...
        if(self.isStopped == True):
            self.updateBearing()
    #not necessary in this class as it will be moved to motor and renamed rotate in future - see isColliding comment
    ...
    #not really necessary, just in to provide a working demo of it moving and detecting collisions
        ...
        for o in self.simulator.getObject():
            ...
            if(o.oType == "barrier"):
                ...
                if((((self.x+50) == o.x) or ((self.x-50) == o.x)) and (self.y+45 >= o.y and self.y-45 <= o.height)):
                    #stops it, pulls it back 1mm then updates location and returns true
                    self.simulator.isStopped = True
                    self.x -= 1
                    self.simulator.setCurrentPosition(self.x,self.y,self.bearing)
                    for observer in self.locationObserverAbstractList:
                        observer.locationUpdated(self.x,self.y, self.bearing)
                    return True
            #same as barrier
                ...
                if(((((self.y)+50) == o.y) and (self.x >= (o.x-o.width) and self.x <= (o.x+o.width)))):
                    ...
                    for observer in self.locationObserverAbstractList:
                        observer.locationUpdated(self.x,self.y, self.bearing)
                    return True
        #same as before for each wall on the course bar maybe 1 or 2
